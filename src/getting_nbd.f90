...
! along with this program; if not, see https://www.gnu.org/licenses/old-licenses/gpl-2.0.en.html
	...
	integer :: search_nbd_x(2), search_nbd_y(2), search_nbd_z(2), i, j, k, l, m, &
		ii, jj, kk, ll, nbd_count, nbd_dis_max
...
!$OMP& nbd_cell_x, nbd_cell_y, nbd_cell_z, nbd_finding_criteria)
	!$OMP DO SCHEDULE(dynamic)
		central_atom :	do k = 1, sc(3) + 2*nbd_cell_x
				...
				do kk = search_nbd_z(1), search_nbd_z(2)
					do jj = search_nbd_y(1), search_nbd_y(2)
						...
						if(ion(0, i, j, k, l).ne.ion(0, ii, jj, kk, ll))then                 
							distance = sqrt(dot_product(&
							ion(6:8, i, j, k, l) - ion(6:8, ii, jj, kk, ll), &
							ion(6:8, i, j, k, l) - ion(6:8, ii, jj, kk, ll)  &
							))
										
							if(abs(nbd_dis(m) - (distance)).le.nbd_finding_criteria)then
								nbd_count = nbd_count +1
								! Storing nbd's ID				
								nn(m, int(ion(0, i, j, k, l)), nbd_count, 0) = ion(0, ii, jj, kk, ll)

								! Storing nbd's indices
								nn(m, int(ion(0, i, j, k, l)), nbd_count, 1) = ii
								nn(m, int(ion(0, i, j, k, l)), nbd_count, 2) = jj
								nn(m, int(ion(0, i, j, k, l)), nbd_count, 3) = kk

								! Storing nbd's atom count
								nn(m, int(ion(0, i, j, k, l)), nbd_count, 4) = ll			
							end if        
                                     		end if
				...
				nn(m, int(ion(0, i, j, k, l)), 0, 0) = nbd_count
