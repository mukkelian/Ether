...
! along with this program; if not, see https://www.gnu.org/licenses/old-licenses/gpl-2.0.en.html
	...
	subroutine update_spin_details(ref1, ref2)
		...
		real(dp) :: rnum, sx, sy, sz, phi
		real(dp), intent(in) :: ref1(5)
		real(dp), intent(out) :: ref2(5)
		...
		if(Ising) then
			...
			if(angle.eqv..FALSE.) then
				...
				call get_random_num(-real(1.0, dp), real(1.0, dp), rnum)
				ref2(3) = rnum					! cos(theta) : S(z)
				call get_random_num(-real(1.0, dp), real(1.0, dp), rnum)
				phi = ref2(5) + (dphi*rnum)			! older_phi + delta*rnum
				ref2(1) = sqrt(1-(ref2(3)**2))*cos(phi)		! S(x)
				ref2(2) = sqrt(1-(ref2(3)**2))*sin(phi)		! S(y)
