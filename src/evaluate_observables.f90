...
! along with this program; if not, see https://www.gnu.org/licenses/old-licenses/gpl-2.0.en.html
	...
	subroutine evaluate_observables(observable_case)
		...
		integer :: i, j, k, l, m
		real(8) :: mag_value, U_mag, chi, U_eng, cv, sample_1, mag_per_site, &
			eng_per_site
		...
		case('eng')
			...
			eng2_avg = eng2_avg + eng_per_site**2.0d0
			eng4_avg = eng4_avg + eng_per_site**4.0d0
		...
		case('mag')
			...
			mag2_avg = mag2_avg + mag_per_site**2.0d0
			mag4_avg = mag4_avg + mag_per_site**4.0d0
		...
		case('avg_mag')
			...
			e_mag_avg(samplei) = mag_avg

			U_mag = 1.0d0 - ((1.0d0/3.0d0)*(mag4_avg/(mag2_avg**2.0d0)))
			s_U_mag = U_mag + s_U_mag
			e_U_mag(samplei) = U_mag
			chi = beta*(mag2_avg - mag_avg**2.0d0)*total_lattice_sites
			s_chi = chi + s_chi
			e_chi(samplei) = chi
		...
		case('avg_eng')
			...
			e_eng_avg(samplei) = eng_avg

			U_eng = 1.0d0 - ((1.0d0/3.0d0)*(eng4_avg/(eng2_avg**2)))
			s_U_eng = U_eng + s_U_eng
			e_U_eng(samplei) = U_eng

			cv = (beta**2.0d0)*(eng2_avg - eng_avg**2.0d0)*total_lattice_sites
			s_cv = cv + s_cv
			e_cv(samplei) = cv

		case('observables_per_sample')

			sample_1 = 1.0d0/(sample*(sample - 1.0d0))
	
			!MAGNETIZATION PER SAMPLE
			s_mag_avg = s_mag_avg/sample
			s_U_mag = s_U_mag/sample
			s_chi = s_chi/sample

			! ERRORS
			err_U_mag = 0.0; err_chi = 0.0; err_mag_avg = 0.0
			...
			err_U_mag = sqrt(sample_1*err_U_mag)
			err_chi = sqrt(sample_1*err_chi)
			err_mag_avg = sqrt(sample_1*err_mag_avg)
	
			!ENERGY PER SAMPLE
			s_eng_avg = s_eng_avg/sample
			s_U_eng = s_U_eng/sample
			s_cv = s_cv/sample

			! ERRORS
			err_U_eng = 0.0; err_cv = 0.0; err_eng_avg = 0.0
			...
			err_U_eng = sqrt(sample_1*err_U_eng)
			err_cv = sqrt(sample_1*err_cv)
			err_eng_avg = sqrt(sample_1*err_eng_avg)
...
!			err_cv_T(itemp) = err_cv; s_U_eng_T(itemp) = s_U_eng; err_U_eng_T(itemp) = err_U_eng
			...
			! ACCEPTANCE RATIOS
			local_obs(14 + li_obs) = acceptance_counting/(tmcs*total_lattice_sites*sample*1.0)*100
