...
! along with this program; if not, see https://www.gnu.org/licenses/old-licenses/gpl-2.0.en.html
	...
	if (opt_stg) then	!getting staggered values of each ion
		...
		if (rank == 0) then
			...
			write(6, *) ''
			write(6, *) '	stg1	for atom1'
			write(6, *) '	stg2	for atom1'
			write(6, *) '	stg3	for atom3'
			write(6, *) '	.		.'
			write(6, *) '	.		.'
			write(6, *) '	.		.'
			write(6, *) '	(Note: Sequence of stg for ith atom should'
	...
	allocate(ion(0:8, sc(1) + 2*nbd_cell_x, sc(2) + 2*nbd_cell_y, sc(3) + 2*nbd_cell_z, lattice_per_unit_cell))
	s_count = 0; ion = 0
